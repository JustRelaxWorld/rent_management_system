...
api.interceptors.response.use(
  (response) => {
    ...
    // Check if the response contains clearToken flag
    if (response.data && response.data.clearToken === true) {
      console.warn('Server requested token removal, clearing auth data');
      localStorage.removeItem('token');
  ...
  (error) => {
    if (error.response) {
      ...
      // Check if the error response contains clearToken flag
      if (error.response.data && error.response.data.clearToken === true) {
        console.warn('Server requested token removal, clearing auth data');
        localStorage.removeItem('token');
        localStorage.removeItem('user');
        window.location.href = '/login';
