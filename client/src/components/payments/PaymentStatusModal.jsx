 ...
 * Uses React Query for real-time polling and Framer Motion for animations
 */
const MpesaPaymentModal = ({
  ...
  // Reset status when modal is opened or new transaction details are received
  useEffect(() => {
    if (show) {
      // Always set initial status to processing when shown
  ...
  // Also reset state when transaction ID changes
  useEffect(() => {
    if (transactionId) {
      setStatus('processing');
      setStatusMessage('');
  ...
  const handleRetry = () => {
    if (onRetry) {
      // Call the retry callback directly
      onRetry();
    }
  };
    ...
    <AnimatePresence mode="wait">
      {show && (
        <Modal 
          ...
          key={`modal-${transactionId || 'default'}`}
        >
          <motion.div
