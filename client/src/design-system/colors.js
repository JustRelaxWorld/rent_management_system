// colors.js - Design system colors

// Base colors
export const colors = {
  primary: {
    50: '#f0f9ff',
    100: '#e0f2fe',
    200: '#bae6fd',
    300: '#7dd3fc',
    400: '#38bdf8',
    500: '#0ea5e9',
    600: '#0284c7',
    700: '#0369a1',
    800: '#075985',
    900: '#0c4a6e',
  },
  secondary: {
    50: '#f0fdfa',
    100: '#ccfbf1',
    200: '#99f6e4',
    300: '#5eead4',
    400: '#2dd4bf',
    500: '#14b8a6',
    600: '#0d9488',
    700: '#0f766e',
    800: '#115e59',
    900: '#134e4a',
  },
  neutral: {
    50: '#fafafa',
    100: '#f5f5f5',
    200: '#e5e5e5',
    300: '#d4d4d4',
    400: '#a3a3a3',
    500: '#737373',
    600: '#525252',
    700: '#404040',
    800: '#262626',
    900: '#171717',
  },
  success: {
    50: '#f0fdf4',
    100: '#dcfce7',
    200: '#bbf7d0',
    300: '#86efac',
    400: '#4ade80',
    500: '#22c55e',
    600: '#16a34a',
    700: '#15803d',
    800: '#166534',
    900: '#14532d',
  },
  warning: {
    50: '#fffbeb',
    100: '#fef3c7',
    200: '#fde68a',
    300: '#fcd34d',
    400: '#fbbf24',
    500: '#f59e0b',
    600: '#d97706',
    700: '#b45309',
    800: '#92400e',
    900: '#78350f',
  },
  error: {
    50: '#fef2f2',
    100: '#fee2e2',
    200: '#fecaca',
    300: '#fca5a5',
    400: '#f87171',
    500: '#ef4444',
    600: '#dc2626',
    700: '#b91c1c',
    800: '#991b1b',
    900: '#7f1d1d',
  },
  info: {
    50: '#eff6ff',
    100: '#dbeafe',
    200: '#bfdbfe',
    300: '#93c5fd',
    400: '#60a5fa',
    500: '#3b82f6',
    600: '#2563eb',
    700: '#1d4ed8',
    800: '#1e40af',
    900: '#1e3a8a',
  },
  // Safaricom M-Pesa colors
  mpesa: {
    green: '#28B445',
    lightGreen: '#e3f5e6',
    darkGreen: '#1a7e31',
    red: '#ED1C24',
    lightRed: '#ffeded',
    darkRed: '#c41017',
  }
};

// Generate CSS variables for all colors
export const generateColorCssVariables = () => {
  let cssVars = '';
  
  for (const colorName in colors) {
    const colorShades = colors[colorName];
    
    if (typeof colorShades === 'object') {
      for (const shade in colorShades) {
        cssVars += `--color-${colorName}-${shade}: ${colorShades[shade]};\n`;
      }
    }
  }
  
  return cssVars;
};

// Function to get a specific color from the system
export const getColor = (colorName, shade) => {
  if (colors[colorName] && colors[colorName][shade]) {
    return colors[colorName][shade];
  }
  
  if (colorName === 'mpesa' && colors.mpesa[shade]) {
    return colors.mpesa[shade];
  }
  
  return null;
}; 