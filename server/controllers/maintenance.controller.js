...
exports.createMaintenanceRequest = async (req, res) => {
  try {
    ...
    // Check if tenant is associated with this property (has an active lease)
    const [leaseCheck] = await pool.execute(
    ...
    // Set image_url if a file was uploaded
    let image_url = null;
    if (req.files && req.files.length > 0) {
      image_url = req.files[0].path.replace(/\\/g, '/');
    }
...
exports.updateMaintenanceRequest = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      req.body.image_url = req.files[0].path.replace(/\\/g, '/');
    }
...
// @desc    Get available properties for maintenance requests (tenant only)
// @route   GET /api/maintenance/available-properties
// @access  Private (Tenant only)
exports.getAvailableProperties = async (req, res) => {
  try {
    ...
    // Get properties where tenant has active leases
    const properties = await Property.findByTenantId(req.user.id);
