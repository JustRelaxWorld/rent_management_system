...
class Maintenance {
  constructor(maintenance) {
    ...
    this.image_url = maintenance.image_url || null;
  }
  ...
  static async create(maintenanceData) {
    try {
      const [result] = await pool.execute(
        ...
        (property_id, tenant_id, title, description, type, status, priority, request_date, images, image_url) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          ...
          maintenanceData.images || null,
          maintenanceData.image_url || null
        ]
      );
